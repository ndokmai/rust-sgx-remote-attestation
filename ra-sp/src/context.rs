...
impl<'a> SpRaContext<'a> {
    pub fn init(mut config: SpConfig, enthropy: &'a mut impl EntropyCallback) 
        -> SpRaResult<Self> {
            ...
            let sp_private_key = SigningKey::new_from_file(
                Path::new(&config.sp_private_key_pem_path), None)?;
